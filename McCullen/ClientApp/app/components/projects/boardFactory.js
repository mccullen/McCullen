	...
	var boardFactory = function (spec) {
		...
		// Private variables
		var initPieces = function (numRows, numColumns) {
			var row = 0;
			var column = 0;
			var pieces = [];
			for (row = 0; row < numRows; row += 1) {
				...
				for (column = 0; column < numColumns; column += 1) {
					pieces[row].push(board.piece.none);
				}
			}
			return pieces;
		};
		...
		var mPieces = initPieces(mNumRows, mNumColumns);
		...
		board.reset = function () {
			mMoves = [];
			mPieces = initPieces(mNumRows, mNumColumns);
